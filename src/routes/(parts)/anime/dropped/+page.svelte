<script lang="ts">
    ...
    export let data, selectedAnime: MediaElement | null = null;
    ...
    {#if anime.status === "paused" && anime.media.status !== "notYetReleased" && (anime.progress?.episode ?? 0) > 0}
      ...
      <BaseCard accent={anime.media.accentColor} background={anime.media.cover.medium} status={anime.media.status} on:click={() => handleCardClick(anime)}>
        ...
        {#if anime.media.status === "airing" && anime.media.episodes?.next}
          <RelativeTimeInfo episode={anime.media.episodes.next} mediaType={anime.media.type} />
        {/if}

        <!-- bottom -->
        <Informations title={anime.media.title.english || anime.media.title.romaji || anime.media.title.native}>
          <AnimeInfo number={anime.media.episodes?.count ?? 0} duration={anime.media.runtime ?? 0}/>
          <DroppedPaused progress={anime.progress?.episode ?? 0} type={anime.status} mediaType={anime.media.type}/>
        </Informations>
      </BaseCard>
