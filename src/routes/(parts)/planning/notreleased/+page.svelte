<script>
    ...
    const animePlanned = anime
        ...
        .sort(
            (a, b) =>
                new Date(a.media.dates.start) - new Date(b.media.dates.start),
        );

    const announcedAnime = anime
        .filter(
            (a) =>
                a.media.status === "NOT_YET_RELEASED" &&
    ...
    {#each announcedAnime as anime}
        ...
        >
            ...
            {#if anime.media.status === "RELEASING"}
                ...
                <RelativeTimeInfo
                    number={anime.nextEpisode.number}
                    timestamp={anime.nextEpisode.timestamp}
