...
console.log("anilist favourites | initialised cache")
const cache = {
    maxTimestamp: null,
    data: null
}
...
export async function GET({ request, url }) {
    ...
    if (url.searchParams.has("clear") && (secrets.apiAuthKey && authHeader === `Bearer ${secrets.apiAuthKey}`)) {
        ...
        cache.data = null;
        console.log("anilist favourites | cleared cache")
    ...
    if ((cache.maxTimestamp && cache.data) && time < cache.maxTimestamp) {
        console.log("anilist favourites | found & served cache")
        return json(cache.data)
    }

    try {
        const data = await getValue("favourites");
        ...
        console.log("anilist favourites | fetched & served from db")
        return json(data)
    } catch (error) {
        console.error(error)
        return json({
