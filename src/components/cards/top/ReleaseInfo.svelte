...
<InformationsTop {catchUp}>
    <span class="next-episode" title={(new Date(episode.timestamp)).toLocaleString(locale)}>
        <RelativeRelease timestamp={episode.timestamp} number={episode.number} mediaType={mediaType}/>
    </span>
</InformationsTop>